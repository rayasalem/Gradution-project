...
import Course from "../db/schemas/courseSchema";
 ...
 export const creatQuiz=async (req :Request,res:Response) =>{
    try{
        ...
        const newQuiz :IQuiz= await new QuizModel({title, description, duration, passingScore});
...
export const updateQuiz= async(req:Request ,res:Response)=>{
    ...
    const updatedQuiz: IQuiz | null = await QuizModel.findByIdAndUpdate(quizId, { title, description, duration, questions, passingScore }, { new: true });
    if (!updatedQuiz) {
        return res.status(404).json({ message: 'Quiz not found' });
      }
